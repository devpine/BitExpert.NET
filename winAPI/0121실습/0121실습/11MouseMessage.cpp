#include <Windows.h>
#include <tchar.h>
#pragma warning(disable:4996)
/*
마우스 메시지 관련.
마우스 부가 정보(클라이언트 영역)
lParam = 클라이언트 영역에서 발생한 마우스의 X, Y좌표
wParam = 클라이언트 영역에서 마우스 버튼의 상태와 키보드 조합 키의 상태 전달MK_
*/
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg) {
	case WM_LBUTTONDOWN: //왼쪽클릭 한 포인트의 좌표 출력
	{
		POINTS pt = MAKEPOINTS(lParam); 
		POINT pt1 = { LOWORD(lParam),HIWORD(lParam) };
		TCHAR buf[50];
		wsprintf(buf, TEXT("(%d:%d)\r\n(%d:%d)"), pt.x, pt.y, pt1.x, pt1.y);
		MessageBox(NULL, buf, TEXT(""), MB_OK);
	}
	return 0;
	case WM_RBUTTONDOWN:
	{
		if (wParam & MK_SHIFT) // Shift키가 눌려져 있는지
		{
			MessageBox(NULL, TEXT("RBUTTON+SHIFT"), TEXT(""), MB_OK);
		}
		else if (wParam & MK_CONTROL) // Control키가 눌려져 있는지
		{
			MessageBox(NULL, TEXT("RBUTTON+CONTROL"), TEXT(""), MB_OK);
		}
	}
	return 0;
	case WM_DESTROY:
		PostQuitMessage(0);
		return 0;
	}
	return(DefWindowProc(hwnd, msg, wParam, lParam));
}

int WINAPI _tWinMain(HINSTANCE hInst, HINSTANCE hPrev,
	LPTSTR lpCmdLine, int nShowCmd)
{
	//1. 윈도우 클래스 만들기
	WNDCLASS wc;
	wc.cbWndExtra = 0;
	wc.cbClsExtra = 0;
	wc.hbrBackground = (HBRUSH)GetStockObject(WHITE_BRUSH);
	wc.hCursor = LoadCursor(0, IDC_ARROW);
	wc.hIcon = LoadIcon(0, IDI_APPLICATION);
	wc.hInstance = hInst;
	wc.lpfnWndProc = WndProc;
	wc.lpszClassName = TEXT("First");
	wc.lpszMenuName = 0;
	wc.style = 0;

	//2. 등록(레지스트리에)
	RegisterClass(&wc);

	//3. 윈도우 창 만들기
	HWND hwnd = CreateWindowEx(0,//WS_EX_TOPMOST
		TEXT("first"),			//클래스 명
		TEXT("Hello"),			// 캡션바 내용
		WS_OVERLAPPEDWINDOW,
		CW_USEDEFAULT, 0, CW_USEDEFAULT, 0, // 초기위치
		0, 0, //부모 윈도우 핸들, 메뉴 핸들
		hInst, // WinMain의 1번째 파라미터(exe주소)
		0); // 생성 인자

	//4. 윈도우 보여주기
	ShowWindow(hwnd, SW_SHOW);
	UpdateWindow(hwnd);

	//5.Message
	MSG msg;
	while (GetMessage(&msg, 0, 0, 0))
	{
		TranslateMessage(&msg);
		DispatchMessage(&msg);
	}
	return 0;
}